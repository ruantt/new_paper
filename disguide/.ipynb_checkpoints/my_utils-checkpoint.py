...
class ContrastiveLoss(nn.Module):
    """
    简单的 SimSiam/SimCLR 风格的对比损失。
    强制要求原图和增强图的特征向量在归一化后尽可能接近（余弦相似度）。
    """
    def __init__(self):
    ...
    def forward(self, x1, x2):
        ...
        # 我们最大化相似度，所以 loss 是负的相似度，或者 1 - similarity
        # 这里使用 1 - mean(cos_sim)
        cos_sim = self.criterion(x1, x2).mean()
        return 1 - cos_sim

def diff_augment(x):
    """
    可微分的数据增强，用于Generator生成的图像。
    包含：随机水平翻转、随机微小平移/裁剪模拟、高斯噪声。
    """
    # 1. 随机水平翻转 (Random Horizontal Flip)
    if random.random() > 0.5:
        x = torch.flip(x, dims=[3])

    # 2. 随机像素偏移 (Random Shift) - 模拟裁剪
    # 简单通过 padding + crop 实现太慢，这里直接用 roll 模拟循环位移，
    # 对于合成的噪声图来说是足够破坏空间结构的。
    if random.random() > 0.5:
        shift_x = random.randint(-4, 4)
        shift_y = random.randint(-4, 4)
        x = torch.roll(x, shifts=(shift_y, shift_x), dims=(2, 3))

    # 3. 添加轻微高斯噪声 (Gaussian Noise)
    # 增加特征提取的抗干扰能力
