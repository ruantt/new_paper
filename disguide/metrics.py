...
class _F:
    ...
    def __call__(self, preds, labels, by_class=False):
        ...
        if by_class:
            return self.metric(preds, labels, task='multiclass', num_classes=num_classes, average=None)
        assert preds.device == labels.device, f"Expected preds and labels on same device. preds:{preds.device} labels:{labels.device}"
        return self.metric(preds, labels, task='multiclass', num_classes=num_classes, average=self.avg)
